...
test('if with function', function(assert) {
  ...
  run(function(){
    ...
    promise = user.validate().then(function(){
      assert.ok(Ember.isEmpty(get(user.get(user.get('errorPropertyName')), 'firstName')));
      ...
      user.validate().then(null, function(){
        assert.deepEqual(get(user.get(user.get('errorPropertyName')), 'firstName'), ["can't be blank"]);
      });
    });
  });
  ...
  run(function(){
    ...
    promise = user.validate().then(function(){
      ...
      user.validate().then(null, function(){
        assert.deepEqual(get(user.get(user.get('errorPropertyName')), 'firstName'), ["can't be blank"]);
        set(user, 'canValidate', false);
        assert.deepEqual(get(user.get(user.get('errorPropertyName')), 'firstName'), []);
      });
    });
  });
