...
test('validations are run on instantiation', function(assert) {
  ...
  assert.deepEqual(get(user, user.get('errorPropertyName') + '.name'), ["can't be blank"]);
  assert.deepEqual(get(user, user.get('errorPropertyName') + '.age'), ["can't be blank", 'is not a number']);
  run(function() {
    user = User.create({name: 'Brian', age: 33});
  });
  assert.ok(get(user, 'isValid'));
  assert.ok(Ember.isEmpty(get(user, user.get('errorPropertyName') + '.name')));
  assert.ok(Ember.isEmpty(get(user, user.get('errorPropertyName') + '.age')));
});
...
test('when errors are resolved', function(assert) {
  ...
  assert.equal(get(user, 'isValid'), false);
  assert.ok(Ember.isEmpty(get(user, user.get('errorPropertyName') + '.name')));
  assert.deepEqual(get(user, user.get('errorPropertyName') + '.age'), ['is not a number']);
  run(function() {
    set(user, 'age', 33);
  });
  assert.ok(get(user, 'isValid'));
...
// test('validations use Ember.I18n.t to render the message if Ember.I18n is present', function() {
  ...
  // assert.deepEqual(get(user, user.get('errorPropertyName') + '.name'), ['muss ausgefüllt werden']);
  // assert.deepEqual(get(user, user.get('errorPropertyName') + '.age'), ['muss ausgefüllt werden', 'ist keine Zahl']);
  // run(function() {
    // set(user, 'age', 'thirty three');
  // });
  // equal(get(user, 'isValid'), false);
  // assert.deepEqual(get(user, user.get('errorPropertyName') + '.age'), ['ist keine Zahl']);
// });
